...
from Functions import Sep_rut, BG_Tender, Mean_BG, Histo, RP, Excel_value, Excel_writter, landau, gaussian, create_folder
...
Ruta=r"C:\Users\genar\Documents\CERN Summer 2025\Carpeta para CERNbox\Spectra_2025_Pablo_Raul_Genaro\N2\100\1_bar\40kV40mA\0V_Again"
...
if Excel_value(excel_path, filters, 'C3kV')!=0:
  ...
  if error_input=='US':
    ...
    Sv = Excel_value(excel_path, filters, 'SV')
    Sc = Excel_value(excel_path, filters, 'SC')
...
plt.plot(df_results['Lambda'], df_phe,  
         label=f'Ar/{Element} {Ar_Concentration}/{int(Concentracion)} {Presion}bar ({Saturation_current}A)', color='blue', linewidth = 0.5)
