...
from Functions import Sep_rut, BG_Tender, Mean_BG, Histo, RP, Excel_value_WP, Excel_writter, landau, create_folder
...
Ruta=r'C:\Users\genar\Documents\CERN Summer 2025\Carpeta para CERNbox\Spectra_2025_Pablo_Raul_Genaro\N2\20\1_bar\40kV40mA\0V_withNO'
...
path_excel_0 = r'C:\Users\genar\Documents\CERN Summer 2025\Carpeta para CERNbox\Spectra_2025_Pablo_Raul_Genaro\Old_Data.xlsx'
path_excel_1 = r'C:\Users\genar\Documents\CERN Summer 2025\Carpeta para CERNbox\Spectra_2025_Pablo_Raul_Genaro\Whole_Data.xlsx'

excel_name = str(input('What archive of data do you want yo use?: '))

validation = False
while validation == False: 
  if excel_name == 'Old_Data':
    path_excel = path_excel_0
    print(f'Using {excel_name}.xlsx')
    validation = True

  elif excel_name == 'Whole_Data':
    path_excel = path_excel_1
    print(f'Using {excel_name}.xlsx')
    validation = True
  
  else:
    print('The name does not exists, please try again')
    excel_name = str(input('Excel name: '))
...
err_SC_candela = 5e-9
Current_candela = Excel_value_WP(path_excel, filters_candela, 'SC')
...
if Excel_value_WP(path_excel,filters, 'SC'):
 
  Saturation_current = Excel_value_WP(path_excel,filters, 'SC')
  Saturation_volt = Excel_value_WP(path_excel,filters, 'SV')
  SC_3kV = Excel_value_WP(path_excel,filters, 'C3kV')
  ...
  if update=='Y':
    ...
    Excel_writter(path_excel,'Ar', Ar_Concentration, 
    ...
    Saturation_current = Excel_value_WP(path_excel,filters, 'SC')
    Saturation_current_3kV = Excel_value_WP(path_excel,filters, 'C3kV')
...
if Excel_value_WP(path_excel,filters, 'C3kV')!=0:
  ...
  if error_input=='US':
    ...
    Sv = Excel_value_WP(path_excel,filters, 'SV')
    Sc = Excel_value_WP(path_excel,filters, 'SC')
